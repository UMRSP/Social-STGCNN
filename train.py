...
def train(epoch, model, loader_train, optimizer, metrics):
    model.train()
    loss_batch = 0 
    batch_count = 0
    is_fst_loss = True
    ...
    for cnt, batch in enumerate(loader_train): 
        ...
        # Get data and send to correct device (CUDA/MPS/CPU)
...
def vald(epoch, model, loader_val, constant_metrics, metrics, checkpoint_dir):
    model.eval()
    loss_batch = 0 
    batch_count = 0
    is_fst_loss = True
    ...
    for cnt, batch in enumerate(loader_val): 
        ...
        # Get data and send to correct device
        batch = [tensor.to(device) for tensor in batch]
        obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel, non_linear_ped,\
         loss_mask, V_obs, A_obs, V_tr, A_tr = batch
...
# Main execution block to prevent multiprocessing spawn errors on Windows
if __name__ == '__main__':
    ...
    for epoch in range(args.num_epochs):
        train(epoch, model, loader_train, optimizer, metrics)
        vald(epoch, model, loader_val, constant_metrics, metrics, checkpoint_dir)
